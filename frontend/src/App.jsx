...
function App() {
    ...
    async (formId) => {
      ...
      try {
        const response = await fetch(
          `${API_BASE}/api/forms/${formId}/submissions`,
        );
  ...
  const handleSelectForm = async (form) => {
    try {
      ...
      if (!response.ok)
        throw new Error("No se pudo cargar la definición del formulario");
      const data = await response.json();
  ...
  const handleAddOption = (fieldIndex) => {
    ...
    updated[fieldIndex].options.push(
      `Opción ${updated[fieldIndex].options.length + 1}`,
    );
    setFormFields(updated);
  };
  ...
  const handleDeleteForm = async (formId, e) => {
    ...
    if (
      !window.confirm(
        "¿Seguro que deseas eliminar esta plantilla y todos sus envíos?",
      )
    )
      return;
    try {
  ...
  const handleSubmitAnswers = async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/forms/${selectedForm.id}/submissions`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      setSubmitSuccess(
        "Respuestas enviadas. El Worker está validando y procesando la información...",
      );
    ...
    <div className="app-container">
      ...
      <header className="app-header">
        <div className="logo-container">
          ...
          <div className="logo-text">
            ...
            <h1>Generador de formularios y agente de envío</h1>
        ...
        <div className="conn-status">
          <div
            className={`indicator ${healthStatus.status === "READY" ? "online" : "offline"}`}
          />
          <span>
            Ecosistema: <strong>{healthStatus.status}</strong>
          </span>
          ...
          <span>
            ...
            <strong
              style={{
                color: healthStatus.db === "UP" ? "#10b981" : "#ef4444",
              }}
            >
              {healthStatus.db}
            </strong>
          ...
          <span>
            ...
            <strong
              style={{
                color: healthStatus.redis === "UP" ? "#10b981" : "#ef4444",
              }}
            >
              {healthStatus.redis}
            </strong>
          ...
          >
            ...
            >
              <h2 className="card-title" style={{ marginBottom: 0 }}>
                Campos del Formulario ({formFields.length})
              </h2>
                ...
                {formFields.map((field, idx) => (
                  <div className="field-builder-card" key={idx}>
                    <div className="field-builder-header">
                      <span className="field-builder-title">
                        Campo #{idx + 1} ({field.name || "sin_nombre"})
                      </span>
                      {formFields.length > 1 && (
                        <button
      ...
      {activeTab === "submissions" && (
        <div className="dashboard-grid">
          ...
          >
            ...
            <div className="card-glass animate-fade-in">
              ...
              {forms.length === 0 ? (
                <div className="empty-state">
                  <p>
                    No hay formularios creados. Dirígete a la pestaña del
                    Diseñador.
                  </p>
                </div>
              ) : (
                  ...
                  {forms.map((f) => (
                    ...
                    >
                      <div style={{ display: "flex", flexDirection: "column" }}>
                        ...
                        >
                          Creado: {new Date(f.created_at).toLocaleDateString()}
                        </span>
                      </div>
                      <button
                        className="btn-danger-sm"
          ...
          <div
            className="card-glass animate-fade-in"
            style={{ animationDelay: "0.1s", height: "fit-content" }}
          >
            ...
            {!selectedForm ? (
              <div className="empty-state">
                <p>
                  Selecciona un formulario de la izquierda para ver el historial
                  de ejecuciones y sus análisis de worker.
                </p>
              </div>
            ) : (
              <div>
                <span className="form-label" style={{ marginBottom: "0.5rem" }}>
                  Envíos Recientes (Form: {selectedForm.title})
                </span>
                {submissions.length === 0 ? (
                  <div className="empty-state">
                    <p>
                      No se registran respuestas para este formulario. Envía uno
                      arriba.
                    </p>
                  </div>
                ) : (
                ...
                {selectedSubmission && (
                  <div
                    className="detail-display animate-fade-in"
                    style={{
                  ...
                  >
                    ...
                    {selectedSubmission.error && (
                      ...
                      >
                        <strong>Error de validación del Worker:</strong>
                        <br />
                        {selectedSubmission.error}
                      </div>
                    )}
                    ...
                    {selectedSubmission.analysis && (
                      <div>
                        ...
                        >
                          Métricas de Negocio Calculadas por Worker
                        </span>
                        <div className="metrics-grid">
                          ...
                          <div className="metric-card">
                            ...
                            <span className="metric-value">
                              {
                                selectedSubmission.analysis.metrics
                                  .total_words_written
                              }
                            </span>
                          </div>
                          <div className="metric-card">
                            ...
                            <span className="metric-value">
                              {
                                selectedSubmission.analysis.fields_summary
                                  .answered
                              }{" "}
                              /{" "}
                              {selectedSubmission.analysis.fields_summary.total}
                            </span>
                          </div>
                          <div className="metric-card">
                            <span className="metric-title">
                    ...
                    <div>
                      ...
                      >
                        Respuestas Recibidas (answers JSON)
                      </span>
                    ...
                    {selectedSubmission.analysis && (
                      <div>
                        <span
                          ...
                          style={{ marginBottom: "0.25rem" }}
                        >
                          Payload de Análisis Completo
                        </span>
                        <pre
                          className="code-block"
                          style={{ color: "#86efac" }}
                        >
                          {JSON.stringify(selectedSubmission.analysis, null, 2)}
                        </pre>
                      </div>
                    )}
                  </div>
